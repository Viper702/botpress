...
module.exports = bp => {
  ...
  const install = async app => {
    ...
    app.use(staticMiddleware(path.join(global.botpressPath, './lib/web')))
    ...
    app.get('*', (req, res, next) => {
      ...
      if (/html/i.test(req.headers.accept) && !/^\/api\//i.test(req.url)) {
        if (req.url && /^\/lite\//i.test(req.url)) {
          return res.sendFile(path.join(global.botpressPath, './lib/web/lite.html'))
        }

        return res.sendFile(path.join(global.botpressPath, './lib/web/index.html'))
      }
      next()
    })
